...
export default class ScrollContent2 extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            listData: null,
    ...
    _renderItem = ({item, index}) => {
        ...
        return <TouchableOpacity style={styles.itemContainer} activeOpacity={0.9} onPress={this._nav2CoinDetail(item)}>
            <Text style={{
                color: Dark_color,
                fontSize: 12
            ...
            <View style={{flexDirection: 'row', alignItems: 'center', paddingVertical: 6}}>
                ...
                <Text>{`¥${item.price_display}`}</Text>
                <Text style={{marginLeft: 'auto', fontSize: 16, marginRight: 6}}>{`¥${item.price_display}`}</Text>
                <View style={{
                    borderRadius: 4,
            ...
            <Text style={{color: Dark_color, fontSize: 12}}>{`$${unitConvert(parseFloat(item.volume_24h))} ${item.anchor}`}</Text>
